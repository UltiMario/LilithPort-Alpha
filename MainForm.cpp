...
void MainForm::Begin()
{
	...
	if(MTOPTION.CONNECTION_TYPE == CT_SERVER && ServerName->Length > 0){
		...
		// ì¡éÍÉTÅ[ÉoÉÇÅ[ÉhîªíË
		if(ServerName[0] == '#'){
			ServerMode = SM_NORA;
			ListView = LV_BLIND;
			me->NAME = gcnew String("Åü");
			me->COMMENT = String::Empty;

			MemberList->Add(me);
			MemberListBackUp->Add(bk);
			listBoxMember->Items->Add(gcnew String("ìΩñºÉÇÅ[Éh"));
		}
		else{
			MemberList->Add(me);
			MemberListBackUp->Add(bk);
		...
		if(MTOPTION.CONNECTION_TYPE == CT_SERVER){
			...
			try{
				...
				*/
				...
				if(address == 0){
					WriteMessage("êERROR: Could not find destination address.\n", ErrorMessageColor);
				...
				if(pd->Divide() == PH_RES_CONNECTION){
					...
					if(ServerName->Length > 0){
						if(ServerName[0] == '+'){
							ServerMode = SM_MIX;
							WriteMessage("çServer is in mixed mode.\n", SystemMessageColor);
						}
						if(ServerName[0] == '@'){
							ServerMode = SM_MATCH;
...
void MainForm::ReceivePackets(IAsyncResult^ asyncResult)
{
	...
	try{
			...
			if(MTOPTION.CONNECTION_TYPE != CT_SERVER){
				...
				try{
					...
					if(id > 0 && i >= MemberList->Count){
						// ÉTÅ[ÉoÇ≈Ç‡ÉNÉâÇ≈Ç‡å©Ç¶Ç»Ç¢èÛë‘
						if(MTINFO.DEBUG){
							form->WriteMessage(String::Format("Unknown({0})ÇÃåüçı\n", id), ErrorMessageColor);
						}
					...
					if(id == MemberList[i]->ID){
						...
						if(rcv[3] == 0xFF){
							...
							if(MTOPTION.CONNECTION_TYPE == CT_SERVER){
								...
								// Ç¢Ç»Ç©Ç¡ÇΩí ím âûã}èàíuVer
								// ë†Ç©ÇÁÇÃÇ›å©Ç¶ÇƒÇ¢ÇÈèÛë‘
								// ÉTÅ[ÉoÇ…ÇÕèÓïÒÇ™Ç»Ç¢ÇÃÇ≈ä‘ê⁄ìIÇ…í ímÇµÇƒÇ‡ÇÁÇ§
								rcv[0] = PH_LOST;
								rcv[1] = 0xFF;
								rcv[2] = 0xFF;
			...
			try{
				...
				if(i < MemberList->Count && ListView == LV_COMMENT){
					if(MemberList[i]->COMMENT->Length > 0){
						...
						form->listBoxMember->Items[i] = gcnew String("Åü");
					}
				}
			}
			finally{
		...
		case PH_REQ_VS:
			...
			// èÄîıéûä‘Ç™í∑Ç∑Ç¨ÇΩèÍçáÇÕêVãKê⁄ë±éÛït
			...
			try{
				...
				/*
					...
					// ç¨ç›ÉTÅ[ÉoÉÇÅ[ÉhÇ»ÇÃÇ≈ÅAé¿çsÉtÉ@ÉCÉãÇÃÉ`ÉFÉbÉN
				...
				*/
					...
					//if(info->FileDescription == "ÇQÇcäiì¨ÉcÉNÅ[Éã2nd."){
						MTINFO.KGT2K = true;
					//}
					//else{
					//	MTINFO.KGT2K = false;
			...
			if((send[1] == MS_FREE)||(send[1] == MS_SEEK)){
				...
				try{
					...
					if(i >= MemberList->Count){
						...
						if(MTOPTION.CONNECTION_TYPE != CT_SERVER){
							// ímÇÁÇ»Ç¢êlÇ™Ç¢ÇΩÇÃÇ≈ÉTÅ[ÉoÇ…ñ‚Ç¢çáÇÌÇπ
			...
			if(NetVS != nullptr && NetVS->SEQUENCE == VS_DATA){
				...
				try{
					if(f >= NetVS->L_FRAME + NetVS->DELAY){
						...
						// ä˘Ç…ÉfÅ[É^Ç™Ç»Ç¢ÇÃÇ≈Ç±ÇÍÇ™Ç≠ÇÈÇ∆ÉQÅ[ÉÄÇ…Ç»ÇÁÇ»Ç¢
						if(MTINFO.DEBUG){
...
// IPéÊìæÉ^ÉCÉÄÉAÉEÉgópÉ^ÉCÉ}Å[
void MainForm::TimerGetIP()
{
	GetIPSleeping = true;
	try{
		// 15ïbÇÆÇÁÇ¢Ç≈É^ÉCÉÄÉAÉEÉg
...
void MainForm::RunSonar()
{
	...
	// 20sÇ…àÍìxÉpÉPÉbÉgÇëóÇËÅA100sà»è„âûìöÇ™Ç»Ç©Ç¡ÇΩÇÁêÿífÇ∑ÇÈ
	while(Ranging){
		SonarSleeping = true;
		try{
...
void MainForm::RunGame(Object^ obj)
{
	...
	REPLAY_INFO ri;
	// dequeÇ™Ç†ÇÈÇΩÇﬂèâä˙âªÇÕéËìÆÇ≈
	ri.CONTROL = 0;
	ri.KEY[0] = ri.KEY[1] = 0xFFFF;
	ri.COUNT[0] = ri.COUNT[1] = 0;

	// ÉfÉBÉåÉCÉVÉ~ÉÖÉåÅ[Ég
		...
		while(WaitForDebugEvent(&de, INFINITE)){
				...
				if(e_code == EXCEPTION_BREAKPOINT){
					...
					case ROUND_END:
						...
						if(vs_end == false){
							...
							// HPéùÇøâzÇµ  Edx:1P_ROUND  Esi:2P_ROUND  Eax:ROUND
			...
			case CREATE_PROCESS_DEBUG_EVENT:
				...
				if(MTINFO.KGT2K){
					...
					// JoyStickÇÕÉGÉâÅ[ãNÇ±Ç∑ÇÃÇ≈ÉIÉtÇ…Ç∑ÇÈ
...
void MainForm::RunVersus()
{
	...
	try{
		...
		if(NetVS->SEQUENCE == VS_PING){
			NetVS->SLEEPING = true;
			try{
				// ç≈í·Ç≈Ç‡ÇPïbä‘Ç…ÉpÉPÇSÇ¬ÇÕó~ÇµÇ¢Ç∆Ç±ÇÎ
				Thread::Sleep(1000);
			}
			catch(ThreadInterruptedException^){
			}
...
*/
				// 5ÉtÉåÅ[ÉÄÇ≤Ç∆Ç…10ÉtÉåÅ[ÉÄï™ëóÇÈ
				if(s_frame + 20 < frame){
...
{
	// ç∂âEìØéûâüÇµã÷é~ if((eax & 3) == 3) eax ^= 2
	...
	if(eax == 0xFFFF){
		...
		while(NetVS->SEQUENCE == VS_DATA){
			...
			if(eax != 0xFFFF){
				...
				i = 0;
				// Ç±Ç±Ç™Ç©ÇÃóLñºÇ»ÉâÉOÇÃê∂éYínÇ≈Ç∑
				// Delay*10 / 2   ÉfÉBÉåÉCÇÃîºÉtÉåÅ[ÉÄï™ÇÕë“ã@ÇµÇƒÇ‡ÇÊÇ≥ÇªÇ§ÅH
...
void MainForm::RunAutoRest() {
	...
	DWORD te;
	int to = MTOPTION.AUTO_REST_TIME;  // É^ÉCÉÄÉAÉEÉgéwíËéûä‘
...
void MainForm::ClearLog(){
	...
	try{
		...
		WriteTime(0, SystemMessageColor);
		WriteMessage("[çDeleted]\n", SystemMessageColor);

		if(MemberList[0]->TYPE == CT_FREE){return;}

		if(MTOPTION.LOG_CLEAR_WITHOUT_WELCOME){ // ÉTÅ[ÉoçêímécÇ∑
			WriteMessage("[MOTD]---------------------\n", SystemMessageColor);
			richTextBoxLog->SelectionFont = gcnew Drawing::Font(richTextBoxLog->Font->FontFamily, richTextBoxLog->Font->Size + 2);
			richTextBoxLog->SelectionColor = TalkMessageColor;
			richTextBoxLog->SelectionBackColor = NoticeBackColor;
